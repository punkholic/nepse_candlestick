...
- `nepse_core/`: the **NEPSE API client** (`nepse_api.py`) + `nepse.wasm` + docs
- `web_ui/`: a small FastAPI web app (candlestick view + snapshot tools)

## Run the web UI

```bash
cd /home/punkholic/random/work/nepselive
python3 -m pip install -r requirements.txt
python3 -m uvicorn web_ui.main:app --host 0.0.0.0 --port 8000
```
...
See `nepse_core/README.md`.

> Note: `nepse_api.py` at repo root is a compatibility shim that re-exports `NepseAPI` from `nepse_core`.
